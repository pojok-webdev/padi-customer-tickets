...
function adm_populate(){
	...
	$ci = & get_instance();
	$sql = "select a.id,a.name,b.id userid,b.username,a.address,a.phone_area,a.phone ";
	$sql.= "from clients a ";
...
function sales_populate($active="1",$status="1",$hide="0"){
	...
	$sql = "select distinct a.id,case when alias is null then a.name else concat(a.name,' (',a.alias,')')  end name,";
