...
function ticket_populate2($custombranch=null){
	...
	$sql.=" where (b.id is not null or c.id is not null or d.id is not null ";
	$sql.=" or e.id is not null or f.id is not null or g.id is not null or h.id is not null) ";

	$sql.=" and create_date>'".$beginquerydate."' ";
...
/*
	...
	$sql.= "downtimestart='".$obj['downtimestart']."', ";
	$sql.= "downtimeend='".$obj['downtimeend']."' ";
	$sql.= "where id=".$obj['id'];
...
function getclientsites(){
	$ci = & get_instance();
	$userbranch = getuserbranches();
	$query = "select a.id,b.id client_site_id,a.name,a.address,c.branch_id ";
